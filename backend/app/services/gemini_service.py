...
**👤 人物特征识别（如果检测到人脸，必须识别）：**
- **年龄估计**：根据面部特征（皱纹、皮肤状态、头发等）估计患者年龄
  - 输出字段：`estimated_age` (整数，例如：75, 68)
  - 如果无法准确估计，可以给出范围（例如：60-70岁）
- **性别识别**：根据面部特征识别性别
  - 输出字段：`gender` ("男"或"女")
  - 如果无法确定，输出 `null`
- **注意**：这些信息用于生成合适的称呼（爷爷/奶奶），请尽量准确识别
...
```json
{
    ...
    "detections": {
        ...
        "facial_analysis": {
            "estimated_age": 75,  // 估计年龄（整数，如果无法估计则为null）
            "gender": "男/女/null",  // 性别识别（如果无法确定则为null）
